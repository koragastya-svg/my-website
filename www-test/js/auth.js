...
const Auth = (() => {
  ...
  function requireAdmin() {
    if (!isLoggedIn()) { window.location.href = '/login?redirect=admin.html'; return false; }
    if (!isAdmin()) {
  ...
  function updateNavAuth() {
    ...
    const loggedInHtml = (session) => `
      ...
      ${session.role === 'admin' ? `<a href="/admin" class="btn-nav-login" style="color:var(--gold);border-color:var(--gold);">관리자</a>` : ''}
      <a href="/mypage" class="btn-nav-login">마이페이지</a>
      <button onclick="Auth.logout()" class="btn-nav-signup">로그아웃</button>
    `;
    const loggedOutHtml = `
      <a href="/login" class="btn-nav-login">로그인</a>
      <a href="/register" class="btn-nav-signup">회원가입</a>
    `;
    const drawerLoggedInHtml = (session) => `
      ...
      ${session.role === 'admin' ? `<a href="/admin" style="display:block;text-align:center;padding:11px 8px;border:1px solid var(--gold);border-radius:4px;color:var(--gold);font-size:0.85rem;margin-bottom:6px;" onclick="closeDrawer()">관리자 페이지</a>` : ''}
      <a href="/mypage" style="display:block;text-align:center;padding:11px 8px;border:1px solid var(--border);border-radius:4px;font-size:0.85rem;margin-bottom:6px;" onclick="closeDrawer()">마이페이지</a>
    ...
    const drawerLoggedOutHtml = `
      <a href="/login" style="display:block;text-align:center;padding:11px 8px;border:1px solid var(--border);border-radius:4px;font-size:0.85rem;margin-bottom:6px;" onclick="closeDrawer()">로그인</a>
      <a href="/register" style="display:block;text-align:center;padding:11px 8px;border-radius:4px;background:var(--navy);color:white;font-size:0.85rem;" onclick="closeDrawer()">회원가입</a>
    `;
...
function renderNavbar(activePage) {
  ...
  <nav class="navbar" id="navbar">
    <div class="navbar__inner">
      ...
      <div class="navbar__desktop-menu" style="display:flex;align-items:center;gap:36px;">
        <a href="/" class="navbar__link ${activePage==='home'?'active':''}">홈</a>
        <a href="/about" class="navbar__link ${activePage==='about'?'active':''}">협회 소개</a>
        <a href="/philosophy" class="navbar__link ${activePage==='philosophy'?'active':''}">철학</a>
        <a href="/service" class="navbar__link ${activePage==='service'?'active':''}">서비스</a>
        <a href="/news" class="navbar__link ${activePage==='news'?'active':''}">소식 &amp; 기사</a>
        <a href="/seminar" class="navbar__link ${activePage==='seminar'?'active':''}">신청</a>
        <a href="/contact" class="navbar__link ${activePage==='contact'?'active':''}">문의</a>
      </div>
  ...
  <div class="navbar__menu" id="navbar-menu" role="dialog" aria-modal="true" aria-label="내비게이션 메뉴">
    ...
    <div class="navbar__auth" id="nav-auth-drawer">
      <a href="/login" style="display:block;text-align:center;padding:11px 8px;border:1px solid var(--border);border-radius:4px;font-size:0.88rem;" onclick="closeDrawer()">로그인</a>
      <a href="/register" style="display:block;text-align:center;padding:11px 8px;border-radius:4px;background:var(--navy);color:white;font-size:0.88rem;" onclick="closeDrawer()">회원가입</a>
    </div>
  </div>`;
  ...
  <footer class="footer">
      ...
      <div class="footer__right-col">
        <div class="footer__menu-section">
          ...
          <div class="footer__menu-grid">
            <a href="/" class="footer__menu-link">홈</a>
            <a href="/about" class="footer__menu-link">협회 소개</a>
            <a href="/philosophy" class="footer__menu-link">철학</a>
            <a href="/service" class="footer__menu-link">서비스</a>
            <a href="/news" class="footer__menu-link">소식 &amp; 기사</a>
            <a href="/seminar" class="footer__menu-link">신청</a>
            <a href="/contact" class="footer__menu-link">문의</a>
          </div>
        </div>
    ...
    <div class="footer__bottom">
      <div class="footer__bottom-links">
        <a href="/privacy" class="footer__bottom-link">개인정보처리방침</a>
        <a href="/terms" class="footer__bottom-link">서비스 이용약관</a>
      </div>
...
function renderBlockScreen(title, message, isMaintenance, timeStr) {
  ...
  const urgentBox = isMaintenance
    ? '<div style="margin-top:24px;padding:14px 18px;background:#f8f4ed;border-radius:8px;border:1px solid #e8dcc8;text-align:center;"><p style="font-size:0.78rem;color:#888;margin:0 0 6px;letter-spacing:0.05em;">긴급 문의 · 오류 제보</p><p style="font-size:0.85rem;color:#444;line-height:1.8;margin:0;">관리자 <strong style="color:#0b1629;">정지수</strong><br><a href="tel:01039992175" style="color:#c8973a;font-weight:600;text-decoration:none;font-size:0.95rem;">010-3999-2175</a></p></div>' +
      '<div style="margin-top:12px;text-align:center;"><a href="/login" style="font-size:0.75rem;color:transparent;text-decoration:none;user-select:none;" tabindex="-1">관리자 로그인</a></div>'
    : '';

  const contactLink = !isMaintenance
    ? '<br><a href="/contact" style="display:inline-block;margin-top:14px;font-size:0.82rem;color:#c8973a;text-decoration:underline;">문의하기</a>'
    : '';
  ...
  overlay.innerHTML =
    ...
    timeBlock +
    '<a href="/" style="display:inline-block;padding:13px 32px;background:#c8973a;color:white;text-decoration:none;border-radius:4px;font-size:0.9rem;font-weight:500;">홈으로 돌아가기</a>' +
    contactLink +
    urgentBox +
    '</div>';
  document.body.appendChild(overlay);
...
(function earlyBlock() {
  try {
    ...
    const allowedPages = ['/', '', '/login', '/admin'];
